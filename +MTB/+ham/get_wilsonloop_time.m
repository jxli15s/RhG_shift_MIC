function [wx1,unk1]=get_wilsonloop_time(obj,M,knum,band1,band2)
...
unk1(:,:,:,knum)=unk1(:,:,:,1);%.*(exp(-1i.*G2*tau'))';
nband=band2-band1+1;
wx1=zeros(knum,nband);
for i=1:knum
    tem=eye(nband);
    for j=1:knum-1
        % tem=tem*unk1(:,band1:band2,j,i)'*unk1(:,band1:band2,j+1,i);
        tem=tem*unk1(:,band1:band2,i,j)'*unk1(:,band1:band2,i,j+1);
    end
    wx1(i,:)=sort(angle(eig(tem))) / pi;
end

% unk2(:,:,knum,:)=unk2(:,:,1,:);%.*(exp(-1i.*G1*tau'))';
unk2(:,:,:,knum)=unk2(:,:,:,1);%.*(exp(-1i.*G2*tau'))';
nband=band2-band1+1;
...
for i=1:knum
    ...
    for j=1:knum-1
        ...
        tem=tem*unk2(:,band1:band2,i,j)'*unk2(:,band1:band2,i,j+1);
    end
