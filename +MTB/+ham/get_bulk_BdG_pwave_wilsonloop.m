function [Energy,kpath,kk]=get_bulk_bands_BdG_pwave(hamiltonian,hopping_r,nbands,nrpts,hkpoints,nk,a,b,mu,gp)
    ...
    parfor i=1:length(kpoints)
        ...
        hk_h=-hk_h.';
        hk_h=hk_h+h_onsite;
        hk_delta=get_hk_delta(gp.ham,gp.hopr,nbands,kpoints(i,:),a);
...
function hk_delta=get_hk_delta(hamiltonian,hopping_r,nbands,kpoint,a)
    nrpts=size(hopping_r,1);
    hamiltonian=reshape(hamiltonian,nbands*nbands,nrpts);
    hk=hamiltonian*exp(1j*((hopping_r*a)*kpoint'));
    hk=reshape(hk,nbands,nbands);
    hk_delta=[zeros(nbands,nbands),hk;...
        hk',zeros(nbands,nbands)];
end
