function gs=get_supercell_wannier_3d(g,n1,n2,n3)
% Slove the Hamiltonian on the K-mesh 
    ...
    cellindex=zeros(n1*n2*n3,3);
    kk=1;
    ll=1;
    for iz=1:n3
        for j=1:n2
            for i=1:n1
                cellindex(n1*n2*(iz-1)+n1*(j-1)+i,:)=[i-1,j-1,iz-1]*g.a/gs.a;
                for k=1:natomc
                    gs.atoms(kk,:) = (g.atoms(k,:)+[i-1,j-1,iz-1])*g.a/gs.a;
                    kk=kk+1;
                end
                for l=1:norbitalc
                    gs.wpos(ll,:) = g.wpos(l,:)+[i-1,j-1,iz-1]*g.a;
                    ll=ll+1;
                end
    ...
    for i=-hop_dim(1):hop_dim(1)
        for j=-hop_dim(2):hop_dim(2)
            for k=-hop_dim(3):hop_dim(3)
                gs.hopr(mm,:)=[i,j,k];
                gs.ham(:,:,mm)=get_t(gs,g,i,j,k,cellindex,norbitalc,norbitals);
                mm=mm+1;
            end
        end
    end
...
function ham=get_t(gs,g,i,j,k,cellindex,norbitalc,norbitals)
    ham=zeros(norbitals,norbitals);
    for l=1:size(cellindex,1)
        for m=1:size(cellindex,1)
            r1=cellindex(l,:);
            r2=cellindex(m,:)+[i,j,k];
