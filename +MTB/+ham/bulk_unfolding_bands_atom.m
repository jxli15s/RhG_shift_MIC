function [Wk,Psik,u_k_super]=get_bulk_unfolding_bands_atom(hamiltonian,hopping_r,nbands,nrpts,hkpoints,nk,g,gs)
    a=gs.a;
    b=gs.b;
    ...
    parfor i=1:length(kpoints)
        ...
        hk=get_hk(hamiltonian,hopping_r,tau,nbands,nrpts,kpoints(i,:),a);
...
function Wk=compute_projection(u_k_super,Psik)
    ...
    for ik=1:numk
        ...
        % Wk(:,ik)=Psik(:,:,ik)' * (diag(u_k_super(:,ik))*Psik(:,:,ik));
    end
end
