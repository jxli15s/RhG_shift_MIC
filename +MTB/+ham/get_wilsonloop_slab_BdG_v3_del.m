function wx=get_wilsonloop_slab_BdG_v3_del(obj,k_mesh_dim,nslab,band1,band2,del_index,mu,delta)
...
nband=band2-band1+1;
wx=zeros(knumx,nband);
parfor i=1:knumx
    unk=zeros(numsub,numsub,knumy);
    for j=1:knumy
        fprintf("Processing on %d %d of %d KPOINTS\n", i, j, knumx*knumy);
        kpoint=[KX(j,i),KY(j,i)];
